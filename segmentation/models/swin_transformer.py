...
class SwinTransformer(nn.Module):
        ...
        if self.use_dense_prediction:
            ...
            for end_idx in idx_crops:
                ...
                if start_idx == 0:
                    ...
                    output_fea = torch.cat((output_fea, _out_fea))
                    npatch.append(N)
                start_idx = end_idx
    ...
    def forward_return_n_last_blocks(self, x, n=1):
...
def swin_tiny_patch4_window7(img_size, is_teacher=False, use_dense_prediction=True):
    model = SwinTransformer(
        ...
        drop_rate=0.,
        attn_drop_rate=0.,
        drop_path_rate= 0.0 if is_teacher else 0.1,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ape=False,
        patch_norm=True,
        use_dense_prediction=use_dense_prediction,
...
if __name__ == '__main__':
    images = torch.randn([10, 3, 448, 448])
    model = swin_tiny_patch4_window7(img_size=224, is_teacher=True, use_dense_prediction=True)
    state_dict = torch.load('/home/thomas/Downloads/temp/swin_tim/geometric.pth')
    msg = model.load_state_dict(state_dict['teacher'], strict=False)
    # _, _, output_fea, _ = model(images)
    output_fea = model.forward_return_n_last_stages(images, n=3)
    for o in output_fea:
        print(o.shape)